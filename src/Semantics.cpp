...
u32 FillFunctionArgumentType(checker *Checker, node *Arg, b32 Error, b32 *OutAutoDef)
{
	if(Arg->Var.TypeNode == NULL)
	{
		...
		{
			RaiseError(false, *Arg->ErrorInfo, "Expected a valid default expression!");
		}
		else
		{
			T = AnalyzeExpression(Checker, Arg->Var.Default);
		}
		if(IsGeneric(T))
		{
			RaiseError(false, *Arg->Var.Default->ErrorInfo, "Unable to evaluate type of expression in function default argument!");
			T = Basic_error;
		}
...
symbol *FindSymbolFromNode(checker *Checker, node *Node, module **OutModule)
{
	switch(Node->Type)
	{
		...
		case AST_SELECTOR:
		{
			...
			{
				RaiseError(false, *Node->ErrorInfo, "Invalid context for automatic enum selector");
				return NULL;
			}
			...
			{
				RaiseError(false, *Node->Selector.Operand->ErrorInfo,
			...
			{
				RaiseError(false, *Node->ErrorInfo, "Couldn't find module %s\n", ModuleName.Data);
				return NULL;
			}
			if(OutModule)
				*OutModule = Import.M;
...
slice<default_value> GetDefaultValues(checker *Checker, slice<node *> Vars, u32 *VarTypes)
{
	...
	ForArray(I, Vars)
	{
		...
		{
			RaiseError(false, *it->ErrorInfo, "All default arguments must be after non default arguments");
		}
	}
	...
	switch(TypeNode->Type)
	{
		...
		case AST_GENSTRUCTTYPE:
		{
			...
			if(StructT->Kind != TypeKind_Struct || (StructT->Struct.Flags & StructFlag_Generic) == 0)
			{
				...
				RaiseError(false, *TypeNode->ErrorInfo, "Expected a generic struct type for generic arguments, instead got %s", GetTypeName(StructT));
				return Basic_error;
			}
		...
		case AST_SELECTOR:
		{
			...
			if(Operand == NULL)
			{
				if(!Error)
					return Basic_error;
				RaiseError(false, *TypeNode->ErrorInfo, "Cannot infer type for `.` selector");
				return Basic_error;
			}
			if(Operand->Type != AST_ID)
			{
				...
				if(Type != Basic_error)
				{
					...
					return Basic_error;
				RaiseError(false, *Operand->ErrorInfo, "Expected module name in selector");
				return Basic_error;
			}
			import Import;
			...
			if(!FindImportedModule(Checker->Imported, SearchName, &Import))
			{
				...
				if(!Error)
					return Basic_error;
				RaiseError(false, *Operand->ErrorInfo, "Couldn't find module `%s`", Operand->ID.Name->Data);
				return Basic_error;
			}
			...
			if(Type == Basic_error)
			{
				if(!Error)
					return Basic_error;
				RaiseError(false, *TypeNode->ErrorInfo, "Type \"%s\" is not defined in module %s", TypeNode->Selector.Member->Data, TypeNode->Selector.Operand->ID.Name->Data);
				return Basic_error;
			}
			return Type;
		} break;
		...
		case AST_UNARY:
		{
			if(TypeNode->Unary.Op == '*')
			{
				...
				if(Pointed == Basic_error)
				{
#if 0
					// @NOTE: Probably not needed?
					if(Error)
						RaiseError(false, *TypeNode->ErrorInfo, "Expected valid type!");
#endif
					return Basic_error;
				}
			...
			else if(TypeNode->Unary.Op == '?')
			{
				...
				{
					if(Error)
						RaiseError(false, *TypeNode->ErrorInfo, "Cannot use ? on a non pointer type.");
					return Basic_error;
				}
...
b32 IsLHSAssignable(checker *Checker, node *LHS)
{
	switch(LHS->Type)
	{
		...
		case AST_LIST:
		{
			...
			{
				if(!IsLHSAssignable(Checker, Item))
					return false;
			}
			return true;
		} break;
		...
		case AST_UNARY:
		{
			if(LHS->Unary.Op != T_PTR && LHS->Unary.Op != T_QMARK && LHS->Unary.Op != T_ADDROF)
				return false;
		...
		case AST_SELECTOR:
		{
			...
			if(TypeIdx == Basic_module)
			{
				...
				{
					RaiseError(false, *LHS->Selector.Operand->ErrorInfo,
							"Invalid use of module");
					return true;
				}
...
u32 GetVariable(checker *Checker, const string *ID, bool *Found)
{
	...
	*Found = Symbol ? true : false;
	return Symbol ? Symbol->Type : Basic_error;
}
...
u32 CreateFunctionType(checker *Checker, node *FnNode, bool Error)
{
	...
	ForArray(Idx, FnNode->Fn.Args)
	{
		if(FnNode->Fn.Args[Idx]->Var.TypeNode == (node *)0x1)
		{
			...
			{
				RaiseError(false, *FnNode->Fn.Args[Idx]->ErrorInfo, "Variadic arguments needs to be last in function type, but it is #%d", Idx);
				return Basic_error;
			}
		}
	}
...
u32 AnalyzeAtom(checker *Checker, node *Expr)
{
	...
	switch(Expr->Type)
	{
		...
		case AST_RUN:
		{
			if(Expr->Run.Body.Count == 0)
			{
				RaiseError(false, *Expr->ErrorInfo, "#run expected an expression");
				break;
			}
			else if(Expr->Run.Body.Count != 1)
			{
				RaiseError(false, *Expr->ErrorInfo, "#run in expression expected a single expression as argument");
			}
		...
		case AST_LIST:
		{
			if(Expr->List.Nodes.Count < 2)
			{
				RaiseError(false, *Expr->ErrorInfo, "Expected expressions between , operator");
				return Basic_error;
			}
			...
			For(Expr->List.Nodes)
			{
				...
				if(IsUntyped(T))
				{
					Ts[At] = UntypedGetType(T);
					FillUntypedStack(Checker, Ts[At]);
		...
		case AST_SWITCH:
		{
			...
			{
				RaiseError(false, *Expr->ErrorInfo, "Type %s cannot be used for a switch expression", GetTypeName(ExprType));
				break;
			}
				...
				else
				{
					if(Case->Case.Value->Type == AST_LIST)
					{
						...
						For(Values)
						{
							...
							{
								RaiseError(false, *Case->Case.Value->ErrorInfo, "Case value must be constant");
							}
		...
		case AST_CALL:
		{
			...
			if(CallTypeIdx == Basic_error)
				return Basic_error;

			const type *CallType = GetType(CallTypeIdx);
			if(!IsCallable(CallType))
			{
				RaiseError(false, *Expr->ErrorInfo, "Trying to call a non function type \"%s\"", GetTypeName(CallType));
				return Basic_error;
			}
...

			{
				...
				ForArray(Idx, Expr->Call.Args)
				{
					...
					if(CallType->Function.ArgCount <= Idx)
					{
						...
						{
							RaiseError(false, *Expr->Call.Args[Idx]->ErrorInfo,
		...
		case AST_EMBED:
		{
			...
			Result = Expr->Embed.IsString ? Basic_string : GetSliceType(Basic_u8);
		} break;
		case AST_TYPEOF:
		{
			if(!Checker->OutOfRun.IsEmpty())
		...
		case AST_CAST:
		{
			...
			Assert(To != Basic_error && From != Basic_error);
		...
		case AST_TYPELIST:
		{
			...
			if(Expr->TypeList.TypeNode == NULL)
			{
				...
				if(TypeIdx == Basic_error)
				{
					RaiseError(false, *Expr->ErrorInfo, "Cannot infer type list type from this expression");
					Result = Basic_error;
					break;
				}
			}
			...
			ForArray(Idx, Expr->TypeList.Items)
			{
				...
				u32 PromotedUntyped = Basic_error;
				switch(Type->Kind)
				{
					case TypeKind_Vector:
					case TypeKind_Slice:
				...
				if(PromotedUntyped != Basic_error)
			...
			if(Type->Kind == TypeKind_Struct)
			{
				...
				{
					RaiseError(false, *Expr->ErrorInfo, "Cannot initialize generic struct without type arguments!");
					Result = Basic_error;
					break;
				}
		...
		case AST_SELECTOR:
		{
			...
			else
			{
				...
				if(EnumT == Basic_error)
				{
					RaiseError(false, *Expr->ErrorInfo, "Cannot infer enum type from this expression");
					Result = Basic_error;
					break;
				}
				...
				switch(Type->Kind)
				{
					...
					case TypeKind_Basic:
					{
						if(HasBasicFlag(Type, BasicFlag_TypeID))
						{
							...
							if(TIdx == Basic_error)
							{
								RaiseError(false, *Expr->ErrorInfo, "Invalid `.`! Cannot use selector on a typeid");
								Result = Basic_error;
								goto AnalyzeAtomEnd;
							}
						...
						else if(IsString(Type))
						{
							...
							{
								RaiseError(false, *Expr->ErrorInfo, "Only .data and .count can be accessed on a string");
								Result = Basic_error;
							}
						}
						else
						{
							if(TypeIdx == Basic_error && HasErroredOut())
							{}
							else
...
ANALYZE_SLICE_SELECTOR:
						...
						{
							RaiseError(false, *Expr->ErrorInfo, "Only .count and .data can be accessed on this type");
							Result = Basic_error;
						}
					} break;
					case TypeKind_Enum:
...
u32 AnalyzeExpression(checker *Checker, node *Expr)
{
	if(Expr->Type == AST_BINARY)
	{
		u32 Left  = Basic_error;
		u32 Right = Basic_error;
		...
		if(X)
		{
			...
			{
				RaiseError(false, *Expr->ErrorInfo, "Invalid operator between pointer and basic type");
				return Basic_error;
			}
			if(X->Pointer.Pointed == INVALID_TYPE)
			{
				RaiseError(false, *Expr->ErrorInfo, "Cannot perform pointer arithmetic on an opaque pointer");
			}
		}
		...
		if(LeftType->Kind == TypeKind_Pointer && HasBasicFlag(RightType, BasicFlag_Integer))
		{
			...
			{
				RaiseError(false, *BinaryExpression->ErrorInfo, "Invalid binary op between pointer and integer!\n"
...
u32 AnalyzeDeclerations(checker *Checker, node *Node, b32 NoAdd = false)
{
	...
	if(Node->Decl.Expression)
	{
		...
		if(Node->Decl.Flags & SymbolFlag_LocalStatic)
		{
			...
			{
				RaiseError(false, *Node->Decl.Expression->ErrorInfo,
		...
		{
			RaiseError(false, *Node->ErrorInfo, "Expression does not give a value for the assignment");
			return Basic_error;
		}
	...
	else
	{
		if(Type == Basic_error)
		{
			RaiseError(false, *Node->ErrorInfo, "Expected either type or expression in variable declaration");
		}
	}
	...
	if(Node->Decl.LHS->Type == AST_LIST)
	{
		...
		if(Node->Decl.LHS->List.Nodes.Count != T->Struct.Members.Count)
		{
			...
			if(Node->Decl.LHS->List.Nodes.Count == 1)
				l = "value";
			RaiseError(false, *Node->ErrorInfo, "Left-hand side expects %d %s but right-hand side gives %d", Node->Decl.LHS->List.Nodes.Count, l, T->Struct.Members.Count);
			return Basic_error;
		}
